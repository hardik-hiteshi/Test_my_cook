...
export class ReturnedProductsService {
  ...
  public async updateReturnedProduct(
    ...
    const updatedReturnedProduct =
      await this.returnedProductRepo.updateReturnedProduct(uniqueId, body);
    if (!updatedReturnedProduct) {
      throw new NotFoundException('ReturnedProduct Not found.');
    }

    return updatedReturnedProduct;
  }

  public async deleteReturnedProduct(uniqueId: string): Promise<object> {
    const deletedReturnedProduct =
      await this.returnedProductRepo.deleteReturnedProduct(uniqueId);
  ...
  public async fetchReturnedProducts(
    ...
    return [];
    // throw new NotFoundException('ReturnedProducts not found.');
  }
